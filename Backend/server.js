...
app.use(cors({origin: 'http://localhost:5173', // Explicitly allow your frontend's origin
    credentials: true}));
...
app.post('/signup', (req, resp) => {
    ...
    //             resp.json("Already have an account")
    //         } else {
    //             users.create({ email: email, name:a name, password: password })
    //                 .then(result => resp.json("  Account Created"))
    //                 .catch(xerr => resp.json(err))
    //         }
    //     }).catch(err => resp.json("failed "))
})
